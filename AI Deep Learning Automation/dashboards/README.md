...
- **[`auto_apply_sizing.py`](auto_apply_sizing.py)** *(~1,000 lines)* — the automation behind the Sizing Reassess / Entry Gate Reassess / pullback cards: it reads accumulated trade evidence and **auto-applies adjustments to `temalimit.cs`'s own constants** within invariant floors, with honest "nothing changed and here's why" messaging and a once-per-day log dampener so it doesn't spam. Full explanation: **[../AUTO-TUNING.md](../AUTO-TUNING.md)**.
